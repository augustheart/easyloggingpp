...
#if (_ELPP_OS_WINDOWS)
		PROCESS_ID_FORMAT_SPECIFIER    ("%pid"),
#endif
...
#if (_ELPP_OS_WINDOWS)
	const std::string PROCESS_ID_FORMAT_SPECIFIER;
#endif
...
#if (_ELPP_OS_WINDOWS)
inline std::string getCurrentProcessId() {
	std::stringstream ss;
...
#if (_ELPP_OS_WINDOWS)
		if (originalFormat.find(constants_->PROCESS_ID_FORMAT_SPECIFIER) != std::string::npos) {
			formatSpec |= constants_->kProcessId;
		}
#endif
...
public:
    RegisteredLoggers(void) :
        ...
        registerNew(new Logger("trivial", constants_));
        ...
        registerNew(new Logger("performance", constants_));
    }
...
private:
    ...
    void buildAndWriteLine(void) {
		...
		//pid
#if (_ELPP_OS_WINDOWS)
		if (f_ & constants_->kProcessId) {
			std::stringstream ss;
			ss << threading::getCurrentProcessId();
			fs_ = constants_->PROCESS_ID_FORMAT_SPECIFIER;
			internal::utilities::LogManipulator::updateFormatValue(fs_, ss.str(), currLine_, constants_);
		}
#endif
        // Date/Time
